...
from flask import redirect, request, render_template, session, abort, Flask, jsonify
...
def refresh_session():
    ...
    user = get_user(session["user_id"], token)

    session["first_name"] = user.get("firstName")
    session["surname"] = user.get("lastName")
    session["email"] = user.get("email")
    print(user.get("attributes", {}).get("phone", [""]))
    session["phone"] = user.get("attributes", {}).get("PhoneNumber", [""])[0]
    session["balance"] = int(user.get("attributes", {}).get("Bank_Currency", [0])[0])
...
def call_api():
    ...
    if 'session_id' in session:
        user = _session_store.get(session['session_id'])
        print(jwt.decode(user.id_token, options={"verify_signature": False}))
        return redirect_with_baseurl('/')
        if not user:
            return redirect_with_baseurl('/')
...
def callback(params):
    ...
    if _jwt_validator and 'id_token' in token_data:
        ...
        user.first_name = temp['given_name']
        user.surname = temp['family_name']
        user.email = temp['email']
        user.phone = temp['phone_number']
        user.user_id = temp['sub']
        session["balance"] = temp['Bank_Currency']
        session["first_name"] = temp.get("given_name")
        session["surname"] = temp.get("family_name")
        session["email"] = temp.get("email")
        session["phone"] = temp.get("phone_number")
        session["user_id"] = temp.get("sub")

    if 'refresh_token' in token_data:
        user.refresh_token = token_data['refresh_token']
    print("Callback called and ran")
    return user
...
def get_admin_token():
    ...
    data = {
        ...
        "client_secret": "tFI5wKIPbAF4RHYq0sryhsymSt8kLD8D"
    }
...
def update_balance(user_id, new_balance):
    ...
    attributes["Bank_Currency"] = [str(new_balance)]
